...
sampleSheet <- yaml::read_yaml(params$yaml)
...
output_dir <- paste0(output_dir, "/", output_suffix, "_FLASHseqONT_output/")

```

```{r load files, include=FALSE, echo = FALSE}

if(file.info(fastq_input)$isdir){
  raw_reads <- parallel::mclapply(list.files(fastq_input, full.names = TRUE, pattern = ".fastq|.fastq.gz"), mc.cores = n_cores, function(x)
        FSnanoporeR::stream_FASTQ_stats(fq = x)) %>%
    bind_rows()
} else {
  raw_reads <- FSnanoporeR::stream_FASTQ_stats(fq = fastq_input)
}
...
Relationship between read length and score (10% of the data if >50K raw reads).

```{r input FASTQ_3, include=TRUE, echo = FALSE, fig.align='center', fig.height=6, fig.width=8}

if(nrow(raw_reads)<50000){
  raw.reads.tmp <- raw_reads
} else {
  raw.reads.tmp <- sample_n(raw_reads, replace = FALSE, size = floor(nrow(raw_reads)/10))
}

ggplot() + 
  geom_density_2d(data = raw.reads.tmp, 
                    aes(y = read_length, x = average_bp_score)) +
  theme_bw(base_size = 18) +
  ylab("Read Length (bp)") + 
  xlab("Average read bp quality (Phred)") +
  scale_y_continuous(breaks = seq(0,20000,1000), labels = scales::comma) +
  scale_x_continuous(breaks = seq(0,90,5), labels = scales::comma)  +
  ggtitle("Read Length vs Quality")
...
Nanopore libraries are prepared by ligation. This method can results in chimeric artefacts where two or more reads are combined. In many cases, dorado/guppy cannot properly split the reads. To detect and split them, BLASTn-Short is used, looking for the PCR adapter sequence: `r pcr_adapter`.
...
blast_search <- vroom::vroom(paste0(output_dir, output_suffix, "_blast_search.txt"), show_col_types = FALSE, col_names = FALSE, delim = "\t", num_threads = n_cores) 
    
blast_search.filter <- filter(blast_search, X12 > floor(0.6*nchar(pcr_adapter)) & X3 >= blast_identity)
knitr::kable(align = "lc",
    tibble(
    "At least 1 PCR Adapter detected (reads, #)" = length(unique(blast_search.filter$X1)),
    "At least 1 PCR  Adapter detected (reads, %)" = round(100*length(unique(blast_search.filter$X1)) / nrow(raw_reads),2),
    "At least 1 PCR  Adapter detected, + strand (reads, #)" = length(unique(blast_search.filter$X1[blast_search.filter$X2 == "PCR_ADAPTER_FWD"])),
    "At least 1 PCR  Adapter detected, + strand, from raw reads (reads, %)" = round(100*length(unique(blast_search.filter$X1[blast_search.filter$X2 == "PCR_ADAPTER_FWD"])) / nrow(raw_reads),2),
    "At least 1 PCR  Adapter detected, - strand (reads, #)" = length(unique(blast_search.filter$X1[blast_search.filter$X2 == "PCR_ADAPTER_REV"])),
    "At least 1 PCR  Adapter detected, - strand, from raw reads (reads, %)" = round(100*length(unique(blast_search.filter$X1[blast_search.filter$X2 == "PCR_ADAPTER_REV"])) / nrow(raw_reads),2)
    ) %>% t()
) 
...
Read length distribution before and after splitting the chimeric reads into segments (0-12kb).
...
if(validate_chimeric == TRUE){
  ...
  knitr::kable(align = "lc",
      tibble(
      "Putative chimeric segments (post-split)" = sum(segs$n),
      "Problematic Reads (banlist, #)" = nrow(chimerc_banlist),
...
```{r Mapping Validation_3, include=FALSE, echo = FALSE, fig.align='center', fig.height=6, fig.width=8}
if(validate_chimeric == TRUE){
...
The barcode primer position is searched in the first/last `r bc_detection_window` bp of the read using vsearch. On the contrary to BLAST, vsearch only reports one hit per read and per query.
...
knitr::kable(align = "lc",
    tibble(
    "Reads with at least one detected barcode (#)" = extractBarcodes_report.tmp$n_reads_with_vsearch_hit,
    ...
    "Reads with at least one detected good barcode (#)" = extractBarcodes_report.tmp$n_reads_correct_bc_total,
...
# vsearch <- read_tsv(paste0(output_dir, output_suffix, "_vsearch.barcodes.txt"), show_col_types = FALSE)

vsearch <- vroom::vroom(paste0(output_dir, output_suffix, "_vsearch.barcodes.txt"), show_col_types = FALSE, delim = "\t", num_threads = n_cores) 
...
```{r vsearch_4, include=FALSE, echo = FALSE, fig.align='center', fig.height=6, fig.width=8}
...
After PCR /sequencing, one barcode can diverge enough and match several barcodes of the whitelist with a the same lv distance. Whenever possible they are corrected using the barcode at the other end.
The first plot depicts the levenstein distances to the whitelist of these barcodes with mismatches. The second one shows how many barcodes from the whitelist match with a given barcode for a specific levenstein distance. The majority of the barcodes should have only one associated barcode. Only those within the bottom left corner are considered.
...
# getUMI_report <- read_tsv(paste0(output_dir, output_suffix, "_get_UMI.report.txt"), show_col_types = FALSE)
getUMI_report <- vroom::vroom(paste0(output_dir, output_suffix, "_get_UMI.report.txt"), show_col_types = FALSE, delim = "\t", num_threads = n_cores) 

if(sampleSheet$UMI_TYPE == "TRIMER"){
    knitr::kable(align = "lc",
      tibble(
       "Reads with a putative UMI detected by vsearch" = sum(getUMI_report$n_vsearch_hit_umidTseq) ,
       "Reads with a putative UMI detected by vsearch, % raw reads" = round(100*sum(getUMI_report$n_vsearch_hit_umidTseq) / n_raw_reads.update,3),
       "Reads with a correct UMI" = sum(getUMI_report$n_umi_first_pass, getUMI_report$n_umi_second_pass),
       "Reads with a correct UMI, % raw reads" = round(100*sum(getUMI_report$n_umi_first_pass, getUMI_report$n_umi_second_pass) / n_raw_reads.update,3),
       "Reads with a correct UMI containing masked bases, % raw reads" = round(100*sum(getUMI_report$n_umi_masked_bases) / n_raw_reads.update,3)
      ) %>% t()
  )
       ...
       "Reads with a UMI detected by vsearch, % raw reads" = round(100*sum(getUMI_report$n_vsearch_hit_umidTseq) / n_raw_reads.update,3),
       "Reads with a correct UMI containing masked bases, % raw reads" = round(100*sum(getUMI_report$n_umi_masked_bases) / n_raw_reads.update,3)
      ) %>% t()
...
knitr::kable(align = "lc",
      tibble(
       "Reads with at least one correct barcode" = nrow(filter(detected_barcodes, !index_unique %in% c("Undetermined", "Chimeric"))),
       ...
       "Reads with chimeric barcode pairs" = nrow(filter(detected_barcodes, index_unique == "Chimeric")),
       ...
       "Reads with undetermined barcode pairs" = nrow(filter(detected_barcodes, index_unique == "Undetermined")),
...
demultiplexing_report <- read_tsv(paste0(output_dir, output_suffix, "_demultiplex.report.txt"), show_col_types = FALSE)
